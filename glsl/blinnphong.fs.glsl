...
void main() {
	...
	vec3 h = normalize((newLightDirection + viewDirection) / 2.0);
	...
	// specular light
	vec3 specular = lightColor * materialSpecularColor * pow(max(dot(h, normal), 0.0), shininess);
	vec3 specularT = lightColor * maskLightColor * pow(max(dot(h, normal), 0.0), shininess);
